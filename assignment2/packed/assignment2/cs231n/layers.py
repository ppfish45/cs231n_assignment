...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        sample_mean = np.mean(x, axis = 0).reshape(1, -1)
        sample_var = np.var(x, axis = 0).reshape(1, -1)
        ...
        #######################################################################
        exp_var = running_var
        out = (x - running_mean) / np.sqrt(exp_var + eps)
        out = out * gamma + beta
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    dx_numerator = dx_hat / cache['x_std']
    dx_u = dx_numerator
    dmu_u = -1 * np.sum(dx_numerator, axis = 0) / N
    dx_denominator = np.sum(dx_hat * (cache['x'] - cache['x_mean']), axis = 0)
    dx_var = -0.5 * np.power(cache['x_var'], -1.5) * dx_denominator
    dx_d = dx_var / N * 2 * (cache['x'] - cache['x_mean'])
    dmu_d = -1 * np.sum(dx_d, axis = 0) / N
    dx = dx_u + dmu_u + dx_d + dmu_d
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * cache['x_norm'], axis = 0)
    dx_hat = dout * cache['gamma']
    dx = (N * dx_hat - np.sum(dx_hat, axis = 0) - cache['x_norm'] * np.sum(dx_hat * cache['x_norm'], axis = 0)) / N / cache['x_std']
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(_H):
        for j in range(_W):
            ...
            pos_W = j * stride
            for k in range(N):
                _x = padded[k, :, pos_H : pos_H + HH, pos_W : pos_W + WW].flatten()
                for p in range(F):
                    _w = w[p].flatten()
                    out[k, p, i, j] = _x.dot(_w) + b[p]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for p in range(F):
            for i in range(_H):
                for j in range(_W):
                    pos_H = i * stride
                    pos_W = j * stride
                    _x = padded[k, :, pos_H : pos_H + HH, pos_W : pos_W + WW]
                    coef = dout[k, p, i, j]
                    db[p] += coef
                    dw[p] += coef * _x
                    dx_padded[k, :, pos_H : pos_H + HH, pos_W : pos_W + WW] += coef * w[p]
        ...
        for k in range(C):
            for i in range(_H):
                for j in range(_W):
                    pos_H = i * stride
                    pos_W = j * stride
                    out[p, k, i, j] = np.max(x[p, k, pos_H : pos_H + p_H, pos_W : pos_W + p_W])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for k in range(C):
            for i in range(_H):
                for j in range(_W):
                    pos_H = i * stride
                    pos_W = j * stride
                    _x = x[p, k, pos_H : pos_H + p_H, pos_W : pos_W + p_W]
                    _ = np.unravel_index(np.argmax(_x), _x.shape)
                    pos = (p, k, pos_H + _[0], pos_W + _[1])
                    dx[pos] += dout[p, k, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    _out, cache = batchnorm_forward(reshaped, gamma, beta, bn_param)
    out = _out.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    reshaped = dout.transpose(0, 2, 3, 1).reshape(-1, C)
    _dx, dgamma, dbeta = batchnorm_backward(reshaped, cache)
    dx = _dx.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
