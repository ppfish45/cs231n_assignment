...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, l2_cache = affine_forward(relu_out, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        loss, softmax_d = softmax_loss(scores, y)
        dout, grads['W2'], grads['b2'] = affine_backward(softmax_d, l2_cache)
        dout = relu_backward(dout, relu_cache)
        dout, grads['W1'], grads['b1'] = affine_backward(dout, l1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, len(sizes)):
            ...
            gamma = 'gamma' + str(i)
            beta = 'beta' + str(i)
            self.params[name_w] = np.random.normal(0, weight_scale, (sizes[i - 1], sizes[i]))
            self.params[name_b] = np.zeros(sizes[i])
            if self.use_batchnorm and i != self.num_layers:
                self.params[gamma] = np.ones(sizes[i])
                self.params[beta] = np.zeros(sizes[i])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        do_cache = [None] * (self.num_layers + 1)
        out = X
        for i in range(1, self.num_layers + 1):
            ...
            gamma = 'gamma' + str(i)
            beta = 'beta' + str(i)
            out, cache[i] = affine_forward(out, self.params[name_w], self.params[name_b])
            if i != self.num_layers:
                if self.use_batchnorm:
                    out, bn_cache[i] = batchnorm_forward(out, self.params[gamma], self.params[beta], self.bn_params[i - 1])
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i != self.num_layers:
                ...
                if self.use_batchnorm:
                    dout, grads[gamma], grads[beta] = batchnorm_backward(dout, bn_cache[i])
            dout, grads[name_w], grads[name_b] = affine_backward(dout, cache[i])
            loss += 0.5 * self.reg * np.sum(self.params[name_w] ** 2)
