...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0, ReLU_coef=0.0):
    """
    ...
    mid_output = X.dot(W1) + b1
    hidden_layer = np.maximum(ReLU_coef * mid_output, mid_output)
    ...
    correct_scores = exp_scr[np.arange(N), y]
    loss = -1 * np.sum(np.log(correct_scores / exp_scr.sum(axis = 1)))
    ...
    #############################################################################
    dscore = exp_scr / np.sum(exp_scr, axis = 1).reshape(-1, 1)
    dscore[np.arange(N), y] -= 1
    dscore /= N
    grads['b2'] = np.sum(dscore, axis = 0)
    mask = (ReLU_coef * mid_output > mid_output).astype(int) * ReLU_coef + (ReLU_coef * mid_output <= mid_output).astype(int)
    grads['W2'] = hidden_layer.T.dot(dscore) + 2 * reg * W2
    grads['b1'] = np.sum(dscore.dot(W2.T) * mask, axis = 0)
    grads['W1'] = X.T.dot(dscore.dot(W2.T) * mask) + 2 * reg * W1
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def predict(self, X):
    ...
    ###########################################################################
    hidden_score = X.dot(self.params['W1']) + self.params['b1']
    hidden_layer = np.maximum(self.ReLU_coef * hidden_score, hidden_score)
